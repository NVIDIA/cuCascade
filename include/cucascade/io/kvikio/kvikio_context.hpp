...
/**
 * @brief Fallback @c ioctx backed directly by kvikIO (@c kvikio::FileHandle).
 *
 ...
 *
 * @note This is cuCascade's cudf-free equivalent of a cudf-datasource-backed
 *       fallback: the io library must not depend on cudf, so it drives kvikIO
 *       directly rather than through @c cudf::io::datasource.
 *
 * Capabilities:
...
class kvikio_context final : public ioctx {
 public:
  ...
  {
    // See ioctx::pre_destroy — drains the cache (if any) while this derived
    // part of the object is still alive.  No reactors to tear down for
    // kvikio_context, but the contract still applies.
    this->pre_destroy();
  }
 ...
 protected:
  /// Backend hook invoked by @c ioctx::open_io_object: open @p path with kvikIO
  /// and record its size.  Applies @c config().compat_mode to the handle when
  /// set.  Throws when the file cannot be opened.
