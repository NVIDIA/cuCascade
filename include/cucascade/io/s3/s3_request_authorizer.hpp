...
/// cuCascade needs only read-only S3 operations; PUT / DELETE etc. are
 ...
 * chain, SSO, ...) without forcing cuCascade to depend on @c aws-sdk-cpp.
 * cuCascade ships @c sigv4_presigned_authorizer as the default impl using
