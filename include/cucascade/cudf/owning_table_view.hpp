...
/**
 * @brief An exclusively-owned owner whose pointee can surrender its columns as
 *        owned @c std::unique_ptr<cudf::column> objects (the canonical case is
 ...
 *
 * Copy-constructible owners (e.g. @c std::shared_ptr<cudf::table>) are
 * deliberately excluded: moving columns out of a shared owner would gut every
 * co-owner. The pointee must also expose @c view() so materialization can
 * verify the base view actually names the owner's columns before moving them
 * out (see the identity guard in @ref owner_backed_view).
 */
template <typename Owner>
concept no_alloc_materializable =
  !std::copy_constructible<Owner> && requires(Owner& owner) {
    { (*owner).release() } -> std::same_as<std::vector<std::unique_ptr<cudf::column>>>;
    { (*owner).view() } -> std::same_as<cudf::table_view>;
  };

/**
 * @brief Whether two column views name the exact same device buffers (recursing
 *        into children), i.e. moving the underlying column is equivalent to
 *        keeping the view.
 */
[[nodiscard]] inline bool shallow_identical(cudf::column_view const& lhs,
                                            cudf::column_view const& rhs)
{
  if (lhs.type() != rhs.type() || lhs.size() != rhs.size() || lhs.offset() != rhs.offset() ||
      lhs.head() != rhs.head() || lhs.null_mask() != rhs.null_mask() ||
      lhs.num_children() != rhs.num_children()) {
    return false;
  }
  for (cudf::size_type i = 0; i < lhs.num_children(); ++i) {
    if (!shallow_identical(lhs.child(i), rhs.child(i))) { return false; }
  }
  return true;
}

/// @copydoc shallow_identical(cudf::column_view const&, cudf::column_view const&)
[[nodiscard]] inline bool shallow_identical(cudf::table_view const& lhs,
                                            cudf::table_view const& rhs)
{
  ...
  for (cudf::size_type i = 0; i < lhs.num_columns(); ++i) {
    if (!shallow_identical(lhs.column(i), rhs.column(i))) { return false; }
  }
  return true;
}
...
class owner_backed_view {
 public:
  ...
  template <typename Owner>
  owner_backed_view(Owner&& owner, cudf::table_view base_view)
    : _model(
  ...
  /// buffers out of the owner when it is @ref no_alloc_materializable and the
  /// base view is column-wise identical to the owner's current view, otherwise
  ...
  struct owner_model final : owner_concept {
      ...
      rmm::device_async_resource_ref mr) override
    {
      if constexpr (no_alloc_materializable<Owner>) {
        // Identity guard: only move columns out when the base view names
        // exactly the owner's current columns — otherwise the base_view-relative
        // selection indices would pull the wrong columns out of the owner's
        // released vector (silent corruption). On mismatch, fall back to copy.
        if (shallow_identical(_base_view, (*_owner).view())) {
      ...
      return materialize_by_copy(selection, stream, mr);
    }

    /// Generic path: copy the selected view into a fresh table (allocates from @p mr).
    [[nodiscard]] std::unique_ptr<cudf::table> materialize_by_copy(
      std::span<const cudf::size_type> selection,
      rmm::cuda_stream_view stream,
 ...
 * @ref detail::owner_backed_view (type-erased owner + column selection), or
 * empty.
 *
 ...
 * @ref detail::owner_backed_view (a @c unique_ptr move, not a copy), after
 * which the column selection is permuted/subset in place. Because of this they
 * are @c const and operate on a @c mutable state member.
 */
class owning_table_view {
 public:
  ...
  /// Take ownership of an already-materialized table. A null @p table is
  /// normalized to the empty state.
  explicit owning_table_view(std::unique_ptr<cudf::table> table)
  {
    if (table) { _state = std::move(table); }
  }
   ...
   * move column buffers out of the owner (provided @p base_view names exactly
   * the owner's columns); otherwise it copies @p base_view.
   */
  template <typename Owner>
  ...
  owning_table_view(Owner&& owner, cudf::table_view base_view)
    : _state(std::make_unique<detail::owner_backed_view>(std::forward<Owner>(owner), base_view))
  {
  }
  ...
  /// True when the handle holds an owned @c cudf::table (materialized state).
  [[nodiscard]] bool is_materialized() const;

  /// Allocation size in bytes of the owned table, or 0 when not materialized.
  /// The size of a view state is only known once materialized.
 ...
 private:
  ...
  mutable std::variant<std::unique_ptr<cudf::table>,
                       std::unique_ptr<detail::owner_backed_view>,
                       std::monostate>
