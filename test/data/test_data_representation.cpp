          ...
          "[cpu_data_representation][gpu_data_representation]")
{
  ...
  SECTION("Multiple GPU representations")
  {
    ...
    gpu_table_representation repr1(
      std::make_unique<cudf::table>(std::move(table1)), *gpu_space, ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});

    auto table2 = create_simple_cudf_table(200, gpu_space->get_default_allocator());
    gpu_table_representation repr2(
      std::make_unique<cudf::table>(std::move(table2)), *gpu_space, ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
...
TEST_CASE("gpu_table_representation with single column", "[gpu_data_representation]")
{
  ...
  gpu_table_representation repr(std::move(table), *gpu_space, ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
...
TEST_CASE("gpu_table_representation clone creates independent copy", "[gpu_data_representation]")
{
  ...
  cucascade::test::expect_cudf_tables_equal_on_stream(
    repr.get_table_view(), cloned->get_table_view(), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
...
static gpu_table_representation wrap_column(
  ...
  ::cuda::stream_ref writer_stream = ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}})
{
  std::vector<std::unique_ptr<cudf::column>> cols;
  cols.push_back(std::move(col));
                                       ...
                                       representation_converter_registry& registry)
{
  ...
  auto repr = wrap_column(
    std::move(col), *const_cast<memory::memory_space*>(gpu_space), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
...
TEST_CASE("Fast converter: STRING column metadata structure", "[fast][string]")
{
  ...
  auto repr = wrap_column(
    std::move(strings_col), *const_cast<memory::memory_space*>(gpu_space), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
  auto host = fast_convert(repr, host_space, registry, stream.view());
  stream.synchronize();
...
TEST_CASE("Fast converter: LIST<INT32> column metadata structure", "[fast][list]")
{
  ...
  auto repr = wrap_column(
    std::move(list_col), *const_cast<memory::memory_space*>(gpu_space), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
  auto host = fast_convert(repr, host_space, registry, stream.view());
  stream.synchronize();
...
TEST_CASE("Fast converter: STRUCT<INT32, FLOAT64> column metadata structure", "[fast][struct]")
{
  ...
  auto repr = wrap_column(
    std::move(struct_col), *const_cast<memory::memory_space*>(gpu_space), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
  auto host = fast_convert(repr, host_space, registry, stream.view());
  stream.synchronize();
...
TEST_CASE("Fast converter: LIST<LIST<INT32>> nested metadata", "[fast][nested]")
{
  ...
  auto repr = wrap_column(
    std::move(outer_list), *const_cast<memory::memory_space*>(gpu_space), ::cuda::stream_ref{cudaStream_t{cudaStreamDefault}});
  auto host = fast_convert(repr, host_space, registry, stream.view());
  stream.synchronize();
                                                         ...
                                                         int num_rows)
{
  auto fill = [&](cudf::data_type dt, std::size_t bytes_per_row, uint8_t pattern) {
    ...
    if (num_rows > 0) {
      ...
      CUCASCADE_CUDA_TRY(
        cudaMemsetAsync(col->mutable_view().head(), pattern, bytes, stream.get()));
    }
    return col;
  };
