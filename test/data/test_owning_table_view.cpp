...
// A move-only owner that satisfies no_alloc_materializable but whose exposed
// view() is NOT the base_view it was paired with — the identity guard must
// reject the move and materialize the base view by copy instead.
struct decoy_owner {
  std::unique_ptr<cudf::table> real;   ///< keeps the base_view's data alive
  std::unique_ptr<cudf::table> decoy;  ///< what (*this) dereferences to
  cudf::table& operator*() const { return *decoy; }
};

// The concept gates the zero-copy move path: exclusively-owned,
// column-surrendering owners only.
static_assert(detail::no_alloc_materializable<std::unique_ptr<cudf::table>>);
static_assert(detail::no_alloc_materializable<decoy_owner>);
static_assert(!detail::no_alloc_materializable<std::shared_ptr<cudf::table>>,
              "shared owners must copy — moving would gut co-owners");
static_assert(!detail::no_alloc_materializable<table_keepalive>);
...
TEST_CASE("owning_table_view is_materialized and alloc_size track state", "[owning_table_view]")
{
  owning_table_view empty;
  REQUIRE_FALSE(empty.is_materialized());
  REQUIRE(empty.alloc_size() == 0);

  owning_table_view handle{tagged_table({10, 20, 30})};
  REQUIRE(handle.is_materialized());
  REQUIRE(handle.alloc_size() > 0);

  // Column ops demote the owned table to a view state: size no longer known.
  std::array<std::size_t, 1> drop{0};
  handle.drop_columns(drop);
  REQUIRE(static_cast<bool>(handle));
  REQUIRE_FALSE(handle.is_materialized());
  REQUIRE(handle.alloc_size() == 0);

  handle.materialize(test_stream(), test_mr());
  REQUIRE(handle.is_materialized());
  REQUIRE(handle.alloc_size() > 0);

  auto out = handle.release(test_stream(), test_mr());
  REQUIRE(out != nullptr);
  REQUIRE_FALSE(handle.is_materialized());
  REQUIRE(handle.alloc_size() == 0);
}

TEST_CASE("owning_table_view null owned table yields an empty handle", "[owning_table_view]")
{
  owning_table_view handle{std::unique_ptr<cudf::table>{}};
  REQUIRE_FALSE(static_cast<bool>(handle));
  REQUIRE(handle.n_columns() == 0);
  REQUIRE(handle.num_rows() == 0);
  REQUIRE_FALSE(handle.is_materialized());
  REQUIRE(handle.alloc_size() == 0);
  REQUIRE(handle.release(test_stream(), test_mr()) == nullptr);
}

TEST_CASE("owning_table_view shared owner materializes by copy (co-owner kept intact)",
          "[owning_table_view]")
{
  auto table         = tagged_table({10, 20});
  auto original_ptrs = data_ptrs(table->view());

  auto shared = std::shared_ptr<cudf::table>(std::move(table));
  auto keep   = shared;  // co-owner that must not be gutted
  owning_table_view handle{std::move(shared), keep->view()};

  auto result = handle.release(test_stream(), test_mr());
  REQUIRE(read_tags(result->view()) == std::vector<std::int32_t>{10, 20});

  // Copying path: fresh buffers, and the co-owner still holds its columns.
  auto result_ptrs = data_ptrs(result->view());
  REQUIRE(result_ptrs[0] != original_ptrs[0]);
  REQUIRE(result_ptrs[1] != original_ptrs[1]);
  REQUIRE(keep->num_columns() == 2);
  REQUIRE(read_tags(keep->view()) == std::vector<std::int32_t>{10, 20});
}

TEST_CASE("owning_table_view identity guard falls back to copy on mismatched base view",
          "[owning_table_view]")
{
  auto real      = tagged_table({10, 20});
  auto decoy     = tagged_table({77, 88});
  auto base_view = real->view();
  auto real_ptrs = data_ptrs(base_view);

  // decoy_owner satisfies the move concept, but its view() is the decoy table,
  // not base_view — moving would surrender the WRONG columns. The guard must
  // detect the mismatch and copy the base view instead.
  owning_table_view handle{decoy_owner{std::move(real), std::move(decoy)}, base_view};

  auto result = handle.release(test_stream(), test_mr());
  REQUIRE(result != nullptr);
  REQUIRE(read_tags(result->view()) == std::vector<std::int32_t>{10, 20});  // base data, not decoy

  auto result_ptrs = data_ptrs(result->view());
  REQUIRE(result_ptrs[0] != real_ptrs[0]);
  REQUIRE(result_ptrs[1] != real_ptrs[1]);
}
