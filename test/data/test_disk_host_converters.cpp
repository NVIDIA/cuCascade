...
TEST_CASE("host_data disk round-trip nullable list column", "[disk][converter][list][nullable]")
{
  ...
  // original column is well-formed. Exercises the null_count > 0 H2D reconstruction path.
  std::vector<int32_t> host_offsets = {0, 2, 5, 5, 8};
  auto const num_lists              = static_cast<cudf::size_type>(host_offsets.size() - 1);
...
TEST_CASE("host_data disk round-trip nullable nested list column",
          "[disk][converter][list][nested][nullable]")
{
