...
struct io_slot {
  ...
  void register_sqe(io_uring_sqe* sqe)
  {
    ...
    io_uring_sqe_set_data64(sqe, static_cast<__u64>(slot_index));
  }
...
void uring_reactor::worker_loop(const std::stop_token& stop_token)
{
  ...
  auto drain_and_submit = [&]() {
      ...
      if (!slot) {
        if (inflight == 0 && !copying_slots.empty() && !std::exchange(wait_for_copy, true)) {
          CUCASCADE_TRY_AND_LOG_EXCEPTION(
            slots[static_cast<std::size_t>(copying_slots.back().slot_index())].event->synchronize(),
    ...
    if (added > 0) { ring.submit(static_cast<std::size_t>(added)); }
  };
  ...
  auto reap_cqes = [&]() {
    ...
    for (auto* cqe : std::span{cqes.data(), n}) {
      ...
      auto& s = slots[static_cast<std::size_t>(si)];
