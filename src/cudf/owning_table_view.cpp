...
void owning_table_view::ensure_view() const
{
  if (auto* table = std::get_if<std::unique_ptr<cudf::table>>(&_state)) {
    ...
    _state    = std::make_unique<detail::owner_backed_view>(std::move(*table), base);
  }
}
...
cudf::table_view owning_table_view::view() const
{
  ...
  if (auto* view = std::get_if<std::unique_ptr<detail::owner_backed_view>>(&_state)) {
    return (*view)->view();
  }
  ...
  std::span<const std::pair<std::size_t, std::size_t>> swaps) const
{
  ...
  ensure_view();
  std::get<std::unique_ptr<detail::owner_backed_view>>(_state)->reorder_columns(swaps);
}
...
void owning_table_view::drop_columns(std::span<const std::size_t> positions) const
{
  if (std::holds_alternative<std::monostate>(_state)) { return; }
  ensure_view();
  std::get<std::unique_ptr<detail::owner_backed_view>>(_state)->drop_columns(positions);
}
...
void owning_table_view::select_columns(std::span<const std::size_t> positions) const
{
  if (std::holds_alternative<std::monostate>(_state)) { return; }
  ensure_view();
  std::get<std::unique_ptr<detail::owner_backed_view>>(_state)->select_columns(positions);
}
...
bool owning_table_view::is_materialized() const
{
  return std::holds_alternative<std::unique_ptr<cudf::table>>(_state);
}

std::size_t owning_table_view::alloc_size() const
{
  if (auto* table = std::get_if<std::unique_ptr<cudf::table>>(&_state)) {
    return (*table)->alloc_size();
  }
  return 0;
}
