  ...
  BatchCopyAccumulator& batch)
{
  ...
  if (meta.type_id == cudf::type_id::LIST) {
    ...
    // LIST offsets must remain INT32 (cudf::size_type) — lists_column_view and all list
    // algorithms read the offsets child as size_type. The INT64 promotion is correct only for
    ...
    // and avoids a purge_nonempty_nulls kernel launch — our serialized data already has
    // consistent null masks — which would otherwise read offsets/child buffers still pending in
    // the batched H2D copies.
